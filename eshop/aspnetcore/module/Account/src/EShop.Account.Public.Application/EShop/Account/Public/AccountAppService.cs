...
using EShop.Account.Public.Emailing;
using EShop.Account.Localization;
using EShop.Account.Public.Phone;
...
namespace EShop.Account.Public
{
    public class AccountAppService : ApplicationService, IAccountAppService, IApplicationService, IRemoteService
	{
		...
		public AccountAppService(IdentityUserManager userManager, IAccountEmailer accountEmailer, IAccountPhoneService phoneService, IIdentityRoleRepository roleRepository)
		{
			...
			base.LocalizationResource = typeof(EShopAccountResource);
		}
		...
		protected virtual async Task<IdentityUser> GetUserByEmail(string email)
		{
			...
			if (user == null)
			{
				throw new BusinessException("EShop.Account:InvalidEmailAddress").WithData("Email", email);
			}
			return user;
		}
		...
		protected virtual async Task CheckSelfRegistrationAsync()
		{
			...
			if (!flag)
			{
				throw new BusinessException("EShop.Account:SelfRegistrationDisabled");
			}
		}
		...
		protected virtual void CheckPhoneNumber(IdentityUser user)
		{
			...
			{
				throw new BusinessException("EShop.Account:PhoneNumberEmpty");
			}
		}
		...
		protected virtual async Task CheckIfPhoneNumberConfirmationEnabledAsync()
		{
			...
			if (!flag)
			{
				throw new BusinessException("EShop.Account:PhoneNumberConfirmationDisabled");
			}
		}
	}
}
