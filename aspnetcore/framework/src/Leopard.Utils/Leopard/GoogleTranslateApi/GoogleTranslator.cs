...
namespace Leopard.GoogleTranslateApi
{
    // 免费：
    // 暂时先暂存在此，免费的 google 翻译 api 调用。但是调用100次就会被限制禁止访问，解封时间 10分钟 ，3小时 等，一天之中解封时间不断变长
    // 翻译需求少的，可以做成队列，慢慢翻译。
    // tkk破解：
    // 之前社区有方案从：https://translate.google.cn/ ，页面的html里面正则匹配出tkk参数，作为参数传入，避免被调用限制。但现在这个网站的html已经做了混淆，
    // 找不到tkk参数了

    // 收费：
    // google云平台：https://console.cloud.google.com/ （控制台需翻墙访问，但翻译api的调用不需要翻墙）  确定语言的翻译，20美元 一百万字符。
    // github[官方sdk]：https://github.com/googleapis/google-api-dotnet-client
    // github[社区版]：https://github.com/vivet/GoogleApi
